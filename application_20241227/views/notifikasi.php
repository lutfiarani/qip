...
<input id="id" class="form-control" name="chat_id" type="hidden" value="-4030815843" >
<input id="waktu" class="form-control" name="waktu" type="text" value="Waktu : <?php echo date("Y-m-d H:i:s");?>">
<input class="form-control" id="po" name="text" type="hidden" value='*AQL Inspection ( <?php echo count($data)?> PO Belum Inspect) * <?php echo "\n";?><?php echo date("Y-m-d H:i:s");?><?php echo "\n";?><?php for($i= 0; $i<count($data); $i++){?><?php echo "\n";?> * <?php echo $data[$i]['PO_NO'];?> --  *<?php echo $data[$i]['FACTORY'];?> <?php }?> '/>
...
<script>
    function kirim_notif(){
        ...
        $.ajax({
            ...
            url     : "https://api.telegram.org/bot6966417462:AAG-sQalSxw-v0wq0K_-ubyViTshnlBNLtY/sendMessage?parse_mode=Markdown",
            async   : true,
            type    : 'POST',
