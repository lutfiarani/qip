    ...
    <script>
      ...
      $(document).ready(function(){
          ...
          function report3_sample(){
            $.ajax({
                url : "<?php echo base_url();?>index.php/C_aql_inspect/report3_guest",
                type: "POST",
                data: {PO_NO:PO_NO , PARTIAL:PARTIAL, LEVEL:LEVEL, REMARK:REMARK, LEVEL_USER:LEVEL_USER},
                dataType: "JSON",
                success: function(data)
          ...
          function report4_listdefect(){
            $.ajax({
                url : "<?php echo base_url();?>index.php/C_aql_inspect/report4_guest",
                type: "POST",
                data: {PO_NO:PO_NO , PARTIAL:PARTIAL, LEVEL:LEVEL, REMARK:REMARK, LEVEL_USER:LEVEL_USER},
                dataType: "JSON",
                success: function(data)
                {
                    ...
                    for(i=0; i<data.code.length; i++){
                        ...
                        for(b = 0; b < (data.report.length/2)-3; b++) {
                            haha = data.report[b]
                    ...
                    for(i=0; i<data.code.length; i++){
                        ...
                        for(b = (data.report.length/2)-3; b < data.report.length; b++) {
                            haha = data.report[b]
                            if((code1.CODE == haha.CODE)&&(haha.REJECT_CODE_NAME != '0')){
                                html += "<tr>"+
                                        "<td>"+haha.CODE_DEFECT+"</td>"+
