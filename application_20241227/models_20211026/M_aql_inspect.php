...
//  $query = ("
    ...
    public function summary_validation_total(){
        ...
        $query = $this->db->query("	SELECT COUNT(PO_NO) AS TOTAL_PO, SUM(TOTAL_QTY) AS TOTAL_QTY, SUM(QTY_INSPECT) AS QTY_INSPECT FROM(
			...
			JOIN [QIP].[dbo].QIP_AQL_DATA_LOG B
			ON A.PO_NO = B.PO_NO
			AND A.PARTIAL = B.PARTIAL
			AND A.LEVEL = B.LEVEL
			AND A.LEVEL_USER = B.LEVEL_USER
    ...
    public function list_repacking(){
        ...
        FROM (
            ...
            WHERE PO_NO IN (SELECT PO_NO FROM [QIP].[dbo].QIP_AQL_DATA_LOG WHERE INSPECT_RESULT='N' )
            AND LEVEL='II'
            AND LEVEL_USER='2'
            GROUP BY PO_NO, PARTIAL
        ) R
        INNER JOIN [QIP].[DBO].QIP_AQL_DATA_LOG T
    ...
    public function bgrade(){
        $query = $this->db->query("SELECT A.SIZE, B.QTY FROM 
                    (SELECT DISTINCT(SIZE) FROM [QIP].[dbo].QIP_AQL_DATA_FIRST WHERE PO_NO='0126504328'
                    AND LEVEL_USER='2') AS A
                    LEFT JOIN 
                    (SELECT * FROM [QIP].[dbo].REPACK_FIRST_DATA
    ...
    public function defect(){
		...
		(SELECT COUNT(*) FROM [QIP].DBO.REPACK_SECOND_DATA G WHERE G.DEFECT = A.CODE_ID ) AS JUMLAH
		FROM 
    ...
    public function delete_inspection(){
        ...
        FROM	[QIP].DBO.QIP_AQL_PO_PARTIAL_INFO AS A
        LEFT	JOIN QIP.DBO.QIP_AQL_DATA_LOG AS B
        ON		A.PO_NO			= B.PO_NO
        AND		A.PARTIAL		= B.PARTIAL 
        AND		A.LEVEL			= B.LEVEL
        AND		A.LEVEL_USER	= B.LEVEL_USER
