...
class M_Inspection extends CI_Model {
	...
	public function search_loadplan($po){
		...
		END AS STATUS_PO2 
		FROM (SELECT B.CELL, D.* , CONVERT(CHAR(10),A.INSPECT_DATE,126) AS INSPECT_DATE_INPUT, C.TYPE,EXPORT_DATE,C.CONTAINER, E.INSPECT_DATE AS INSPECT_DATE_AQL, F.STATUS_PO,
	...
	CASE WHEN E.INSPECT_DATE IS NULL AND BALANCE_CRTON='0' THEN 'WAITING'
        ...
        FROM (
                   SELECT t.PO_NO, t.INSPECT_DATE, r.MaxTime
					...
					INNER JOIN  [QIP].[dbo].[T_EXPORT] t
					ON t.PO_NO = r.PO_NO AND t.LMNT_DTTM = r.MaxTime) AS C
                    ON A.PO_NO=C.PO_NO
                    LEFT JOIN (
                    ...
                    LEFT JOIN 
                    (SELECT PO_NO, INSPECT_DATE  FROM QIP_AQL_DATA_FIRST
                    WHERE CONVERT(CHAR(6),LMNT_DTTM,112) >= CONVERT(CHAR(6), GETDATE(),112)-1
                    GROUP BY PO_NO, INSPECT_DATE) AS E
                    ON A.PO_NO=E.PO_NO
                    LEFT JOIN (SELECT t.PO_NO, t.STATUS_PO, r.MaxTime
					FROM (
	...
	public function detail_po($po){
		$query = $this->db->query("
						...
						SELECT ZCCELL, A.PO_NO, A.MODEL_NAME, B.CUST_NO, B.DESTINATION, ART_NO, QTY_TOTAL, TOTAL_CARTON, PD, SDD, TYPE, CONTAINER, EXPORT_DATE, INSP_DATE, BALANCE  FROM 
						(
	...
	public function list_inspection(){
		$query = $this->db->query("SELECT A.*, B.STATUS_PO, B.MaxTime FROM [QIP].[dbo].[T_INSPECTION] AS A
		LEFT JOIN (SELECT t.PO_NO, t.STATUS_PO, r.MaxTime
			FROM (
				SELECT PO_NO, MAX(STATUS_DATE) as MaxTime
				FROM [QIP].[dbo].[T_EXPORT_IC]
			...
			INNER JOIN [QIP].[dbo].[T_EXPORT_IC] t
			ON t.PO_NO = r.PO_NO AND t.STATUS_DATE = r.MaxTime) AS B
			ON A.PO_NO=B.PO_NO
