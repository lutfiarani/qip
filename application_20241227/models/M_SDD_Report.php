...
class M_SDD_Report extends CI_Model {
    ...
    public function list_SDD(){
        $query = $this->db->query("
                        SELECT DISTINCT(CONVERT(CHAR(10), SDD, 120))  AS ZHSDD
                        FROM [MEShs].dbo.THPRODHISPO WITH (NOLOCK)
                            WHERE CONVERT(CHAR(10), SDD, 120) >= CONVERT(CHAR(10), GETDATE(),120)
                            ORDER BY CONVERT(CHAR(10), SDD, 120) ");
