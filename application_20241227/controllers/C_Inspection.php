...
class C_Inspection extends CI_Controller {
	...
	public function upload_inspection_result(){
		$level = $this->session->userdata('LEVEL');
		if($level){
			// $factory = $this->session->userdata('FACTORY');
			$datasub['tingkat'] = $level;
			$datasub['username'] = $this->session->userdata('USERNAME');
			$data['pagetitle']="UPLOAD INSPECTION RESULT";
			$subdata['formtitle'] = "UPLOAD INSPECTION RESULT";
			$subdata['query'] = $this->M_Inspection->list_mes_inspect();
	...
	public function list_mes_inspect(){
		...
		$list = $this->M_Inspection->list_mes_inspect();
		$data = array();
		$no = $start;
		foreach($list->result() as $a){
			$no++;
	...
	public function import_mes()
	{
		if(!empty($_FILES['file']['name'])) { 
			...
			$arrayCount = count($allDataInSheet);
			$flag = 0;
			$createArray = array('PO_NO','INSPECT_DATE');
			$makeArray = array('PO_NO'=>'PO_NO','INSPECT_DATE'=>'INSPECT_DATE');
			$SheetDataKey = array();
			foreach ($allDataInSheet as $dataInSheet) {
				foreach ($dataInSheet as $key => $value) {
					if (in_array(trim($value), $createArray)) {
			...
			if ($flag == 1) {
				for ($i = 2; $i <= $arrayCount; $i++) {
					...
					$fetchData[] = array( 'PO_NO' => $PO_NO, 'INSPECT_DATE' => $INSPECT_DATE,'LMNT_DTTM' => $UPLOAD);
				}   
				$data['dataInfo'] = $fetchData;
				$this->M_Inspection->setBatchImport($fetchData);
