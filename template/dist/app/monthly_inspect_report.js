    ...
    function show_table(){
     ...
     tableInspect =  $('#inspectPOList').DataTable({
               ...
               //n1, n2 ... are number formats; s1, s2, ... are styles
                   ...
                   //set lenght of some columns: col number: length (excl. first column)
                      ...
                      for ( i=2; i < rows.length; i++ ) {
                        if ( $('row:eq('+i+') c[r^="W"] t', sheet).text() == 'FAIL'){
                          a = i+1;
                          $('row:nth-child('+a+') c', sheet).attr('s','53');
                          $('row:nth-child('+a+') c:nth-child(24)', sheet).attr('s','52');
                        }else{
                          $('row:eq('+i+') c', sheet).attr( 's', centerBorder );
                        }
               ...
               "aoColumnDefs": [{ "aTargets": [0], "bSortable": true },
                                 ...
                                 "aaSorting": [[0, 'asc']],
               "ajax":{
                   url : base_url+"/qip/index.php/C_aql_inspect/monthly_report_inspection_",
