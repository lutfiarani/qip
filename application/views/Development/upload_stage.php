  ...
  <!--MODAL DELETE------------>
            ...
            <div class="modal-dialog modal-lg">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">Update Fail Status</h4>
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                    </div>
                    <div class="modal-body">
                    <form method="post" id="import_file2" enctype="multipart/form-data">
                    <label for="exampleInputFile"><span id="updateApa"></span></label>
                        <div class="form-group">
                           <label for="exampleInputFile">File input</label>
                           <div class="input-group">
                           <div class="custom-file">
                                <input type="file" class="custom-file-input" id="myPdf2" name="file2" required accept=".pdf">
                                <label class="custom-file-label" for="file">Choose file</label>
                           </div>
                        ...
                        <canvas id="pdfViewer2"  name="pdfViewer2"></canvas>

                        <div class="card-footer float-end">
                          <p align="right">
                            <input type="submit" name="import2" value="Upload File" class="btn btn-primary btn-sm" />
                          </p>
                        </div>
                    </div>
    ...
    $(document).ready(function(){
                ...
                success: function(data){
                    ...
                    for(i=0; i<data.length; i++){
                        html +='<tr>'+
                                ...
                                '<td><button type="button" class="btn btn-success btn-xs preview" data-toggle="modal"  data-file="'+data[i].DOKUMEN+'">Preview</button>   '+
                                ...
                                if(data[i].STATUS=='FAIL'){
                                    html +='<td><button type="button" class="btn btn-block btn-warning btn-xs pass"  data-file="'+data[i].DOKUMEN+'">PASS</button></td>';
                                }else if((data[i].STATUS=='PASS')){
                                    html += '<td></td>';
                                }else if((data[i].STATUS=='RE-TRIAL PASSED')){
                                    html += '<td>Update to Re Trial Passed at '+data[i].UPDATED_AT+'<br>'+
                                            '<button type="button" class="btn btn-success btn-xs preview" data-toggle="modal"  data-file="'+data[i].DOKUMEN_UPDATE+'">Preview</button>   '+
                                            '<a class="btn btn-info btn-xs download" href="'+alamat+'/'+data[i].DOKUMEN_UPDATE+'"  download>Download</a>   '+
                                            '</td>';
                                }
                               no++;
                       html +=     '</tr>';
        ...
        $(document).on('click','.pass',function(){
            ...
            $.ajax({
              type : "POST",
              url  : "<?php echo site_url('C_dev_stage/get_file')?>",
              dataType : "JSON",
              data : {nama_file:nama_file},
              success: function(data){
                 $('#modalUpdate').modal('show');
                 $('[name="id_file"]').val(nama_file);
                 html ='';
                 html+= '<div class="alert alert-warning alert-dismissible">';
                 html += '<h5><i class="icon fas fa-ban"></i> Alert!</h5>';
                 html += "<p>Update Status Fail for Model "+data.MODEL_INPUT+"uploaded at "+data.CREATED_AT
                 $('#updateApa').html(html);
        ...
        $('#import_file2').submit(function(e){
            var model = $('#model').val();
            e.preventDefault(); 
                $.ajax({
                    url:'<?php echo site_url('C_dev_stage/upload_edit')?>',
                    method:"post",
                    data:new FormData(this),
                    processData:false,
                    contentType:false,
