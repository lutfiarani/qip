...
class C_aql_inspect extends CI_Controller {
	...
	public function confirm_inspector(){
		...
		if (($LEVEL_USER == 3) || ($LEVEL_USER == 2) || ($LEVEL_USER == 6) ){
			...
			$data = $this->M_aql_inspect->confirm_inspector($PO_NO, $PARTIAL, $REMARK, $LEVEL, $USERID, $LEVEL_USER, $FLAG, $COMMENT, $LEVEL_U);
		} else if ($LEVEL_USER == 4){
			$FLAG = 2;
			$data = $this->M_aql_inspect->confirm_inspector_repre($PO_NO, $PARTIAL, $REMARK, $LEVEL, $USERID, $LEVEL_USER, $FLAG , $LEVEL_U);
	...
	public function summary_third(){
		sesscheck();
		$data1 = $this->M_aql_inspect->summary_third_total();
		$data2 = $this->M_aql_inspect->summary_third_defect();
	...
	public function summary_third_fac(){
        ...
        $data = $this->M_aql_inspect->summary_third_fac($tanggal)->result();
	...
	public function summary_third_date(){
		$tanggal = $this->input->post('tanggal');
        $data = $this->M_aql_inspect->summary_third_date($tanggal)->result();
	...
	public function summary_validation(){
		sesscheck();
		$data1 = $this->M_aql_inspect->summary_validation_total();
		$data2 = $this->M_aql_inspect->summary_validation_defect();
		echo json_encode(array('haha1'=>($data1->row_array()), 'haha2'=>($data2->result())));
	...
	public function summary_inspection_(){
		$this->M_aql_inspect->summary_inspection();
		$data_tampil = $this->M_aql_inspect->summary_inspection_view();
		$data_tambah = $this->M_aql_inspect->summary_inspection_sum();
		$this->M_aql_inspect->drop_summary_inspection();
	...
	public function input_repacking2(){
		...
		$coba 	= $this->M_aql_inspect->list_repacking();
		$data 	= array();
		$no		= $start;
	...
	public function import_repacking()
	{
		if(!empty($_FILES['file']['name'])) { 
			...
			if ($flag == 1) {
				...
				$this->M_aql_inspect->setBatchImport($fetchData);
				$insert = $this->M_aql_inspect->importRepacking();
				// echo $fetchData;
	...
	public function import_defect()
	{
		if(!empty($_FILES['file']['name'])) { 
			...
			if ($flag == 1) {
				...
				$data['dataInfo'] = $fetchData;
				$this->M_aql_inspect->setBatchImport($fetchData);
				$insert = $this->M_aql_inspect->importDefect();
				// echo $fetchData;
				echo "IMPORT DATA BERHASIL";
	...
	public function view_repacking($PO){
		...
		$data['po_info']	= $this->M_aql_inspect->po_info();
		$data['bgrade'] 	= $this->M_aql_inspect->bgrade();
		$data['repacking'] 	= $this->M_aql_inspect->repacking();
		$data['cgrade'] 	= $this->M_aql_inspect->cgrade();
		$data['defect'] 	= $this->M_aql_inspect->defect();
		$data['apa'] 		= $this->M_aql_inspect->defect_count();
	...
	public function delete_inspection_(){
        ...
        $length	 		= intval($this->input->get("length"));
       	...
       	$inspect_list  = $this->M_aql_inspect->delete_inspection();
		$data          = array();
		$no            = $start;
	...
	public function delete_inspection__(){
		...
		$this->M_aql_inspect->delete_inspection_($po, $partial, $level, $level_user, $remark);
	...
	public function import_pivot(){
		...
		// if($level){
			...
			// $subdata['query'] = $this->M_aql_inspect->list_import_pivot();
	...
	public function import_pivot_()
	{
		if(!empty($_FILES['file']['name'])) { 
			...
			if ($flag == 1) {
				...
				$data['dataInfo'] = $fetchData;
				$this->M_aql_inspect->setBatchImport($fetchData);
				$insert = $this->M_aql_inspect->importPivot();
				// echo $fetchData;
				$this->M_aql_inspect->send_to_mes();
				$this->M_aql_inspect->change_status();
				echo "IMPORT DATA BERHASIL";
	...
	public function delete_import_pivot(){
		$delete = $this->M_aql_inspect->delete_import_pivot();
